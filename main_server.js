...
// 1. Dapatkan daftar origin yang diizinkan
const allowedOrigins = [
  'https://fetoken.vercel.app', 
  'http://localhost:3000',
  'https://familybrs.vercel.app'
...
// 2. GUNAKAN PACKAGE CORS RESMI (Hapus res.setHeader manual!)
app.use(cors({
  origin: function (origin, callback) {
    // Izinkan jika tidak ada origin (Postman/Server-to-server) atau jika ada di whitelist / netlify
    if (!origin || allowedOrigins.includes(origin) || origin.endsWith('.netlify.app')) {
      callback(null, true);
    } else {
  ...
  credentials: true, // Wajib true untuk cookie withCredentials
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'path', 'x-path', 'X-Requested-With']
}));

// 3. MATIKAN CROSS-ORIGIN RESOURCE POLICY DI HELMET
app.use(helmet({
...
if (!process.env.VERCEL) {
  ...
  app.listen(port, '0.0.0.0', () => console.log('BE JALAN DI PORT ' + port));
}
