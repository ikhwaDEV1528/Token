...
async function Login(req, res) {
  ...
  try {
    // 🧪 TES BYPASS DB (Komentar sementara query DB)
    // const [rows] = await db.query(`SELECT * FROM admin WHERE username = ?`, [username]);

    // Data dummy untuk membuktikan jalur Vercel -> Railway
    const user = { id_admin: 1, username: 'admin', role: 'Admin' };

    const payload = { id_admin: user.id_admin, username: user.username, role: user.role };
    const TokenAccses = jwt.sign(payload, 'RAHASIA_GW', { expiresIn: '15m' });
    const TokenReload = jwt.sign(payload, 'RAHASIA_GW', { expiresIn: '2h' });
